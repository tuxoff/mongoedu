import sys
import bottle
import pymongo
from bottle import run


@bottle.get("/hw1/<n>")
def get_hw1(n):
    # connnecto to the db on standard port
    connection = pymongo.MongoClient("mongodb://localhost")

    n = int(n)

    db = connection.m101  # attach to db
    collection = db.funnynumbers  # specify the colllection

    magic = 0

    try:
        iter = collection.find({}, limit=1, skip=n).sort('value', direction=1)
        for item in iter:
            return str(int(item['value'])) + "\n"
    except:
        print ("Error trying to read collection:", sys.exc_info())


@bottle.get('/hello')
def hello():
    return bottle.template('basic', {"content": "Hello World real time1!"})


@bottle.get('/')
def index():
    return bottle.template('basic', {"content": ''})


@bottle.get('/form')
def show_form():
    return bottle.template('form')

@bottle.post('/form')
def process_form():
    value = bottle.request.forms.get('val')
    return bottle.template('form_result', {'content': value})

run(reloader=True)
bottle.debug(True)
bottle.run(host='localhost', port=8080)